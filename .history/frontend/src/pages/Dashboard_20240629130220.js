...
const Dashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await api.post('/entregas', { ...form, tempoDecorrido: 0 }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setForm({
        nomeEntregador: '',
    ...
    <div>
      ...
      <ul>
        {entregas.filter(entrega => entrega.status !== 'Finalizada').map((entrega) => (
          <li key={entrega._id}>
            <span>{entrega.nomeEntregador} - {entrega.cliente} - {entrega.volume} - {entrega.status} - {entrega.tempoEstimado} minutos - Tempo Em Andamento: {formatTime(entrega.tempoDecorrido)}</span>
