...
const Dashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setEntregas(prevEntregas => prevEntregas.map(entrega => entrega._id === response.data._id ? { ...response.data, tempoIdeal } : entrega));
      setForm({
        nomeEntregador: '',
        cliente: '',
        volume: '',
    ...
    <div>
      ...
      <ul>
        {entregas.filter(entrega => entrega.status !== 'Finalizada').map((entrega) => (
          <li key={entrega._id}>
            <span>{entrega.nomeEntregador} - {entrega.cliente} - {entrega.volume} - {entrega.status} - {entrega.tempoEstimado} minutos - Tempo Em Andamento: {formatTime(entrega.tempoDecorrido)} - Tempo Ideal: {formatTime(entrega.tempoIdeal)}</span>
