...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchEntregas = async () => {
      try {
        ...
        console.log('Token fetched:', token); // Log do token
        const response = await api.get('/entregas', {
          headers: { Authorization: `Bearer ${token}` }
        });
        console.log('Entregas fetched:', response.data); // Log das entregas
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      console.log('Sending token:', token); // Log do token ao enviar
      const response = await api.post('/entregas', form, {
        headers: { Authorization: `Bearer ${token}` }
      });
      console.log('Entrega added:', response.data); // Log da resposta
