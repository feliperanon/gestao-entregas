...
import {
  Container, Typography, TextField, Button, Select, MenuItem, InputLabel, FormControl, Box, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, IconButton, Grid
} from '@mui/material';
...
const Analise = () => {
  ...
  return (
    <StyledContainer>
      <Typography variant="h4" gutterBottom>Análise de Entregas</Typography>
              ...
              <TableBody>
                {entregas.filter(entrega => entrega.status === 'Entrega').map((entrega) => (
                  <TableRow key={entrega._id}>
                    <TableCell>{entrega.nomeEntregador}</TableCell>
                    <TableCell>{entrega.cliente}</TableCell>
                    <TableCell>{entrega.status}</TableCell>
                    <TableCell>{entrega.volume}</TableCell>
                    <TableCell>{entrega.tempoEstimado} minutos</TableCell>
                    <TableCell>{entrega.tempoDecorrido} minutos</TableCell>
                    <TableCell>
