...
const Dashboard = () => {
  ...
  const handleFinalize = async (id) => {
    try {
      ...
      // Atualiza a lista de entregas para remover a entrega finalizada
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <select name="nomeEntregador" title="Selecione o Entregador" value={form.nomeEntregador} onChange={handleChange} required>
          <option value="">Selecione o Entregador</option>
          {entregadores.map((entregador) => (
            <option key={entregador._id} value={entregador.nome}>{entregador.nome}</option>
          ))}
        </select>
        <select name="cliente" title="Selecione o Cliente" value={form.cliente} onChange={handleChange} required>
          <option value="">Selecione o Cliente</option>
          {clientes.map((cliente) => (
            <option key={cliente._id} value={cliente.nome}>{cliente.nome}</option>
          ))}
        </select>
        <select name="status" title="Selecione o Status" value={form.status} onChange={handleChange} required>
      ...
      <ul>
        {entregas.filter(entrega => entrega.status === 'Em andamento').map((entrega) => (
          <li key={entrega._id}>
